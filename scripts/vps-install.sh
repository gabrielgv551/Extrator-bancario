...
const PATCH_POLL_INTERVAL_MS = 2000;   // intervalo entre polls
const PATCH_MAX_WAIT_MS = 120000;      // timeout máximo: 2 minutos
const MIN_TIME_BETWEEN_PATCHES_MS = 5 * 60 * 1000; // 5 minutos entre PATCHes
...
async function getPluggyItem(itemId) {
  return pluggyGet(`/items/${itemId}`).catch(() => null);
}

/**
 * Aguarda o item sair do status UPDATING, fazendo polling.
 * Retorna o item final ou null se timeout.
 */
async function waitForItemReady(itemId, maxWaitMs = PATCH_MAX_WAIT_MS) {
  ...
  while (Date.now() - start < maxWaitMs) {
    ...
    if (status !== 'UPDATING') {
      // Status inesperado (ERROR, LOGIN_ERROR, etc)
      console.log(`    ⚠ item ${itemId}: status inesperado "${status}" (execution: ${executionStatus})`);
      return item;
    }
    
    // Ainda UPDATING, aguardar
    await new Promise(r => setTimeout(r, PATCH_POLL_INTERVAL_MS));
  }
  
  console.log(`    ⚠ item ${itemId}: timeout após ${maxWaitMs}ms aguardando sync`);
  return null;
}
...
async function upsertBatch(table, clientId, pluggyItemId, transactions) {
  ...
  // Isso evita duplicatas quando a Pluggy muda o ID de PENDING para POSTED
  const duplicates = [];
  const newTxs = [];
  ...
  for (const tx of transactions) {
    ...
    if (rows.length > 0) {
      // Já existe transação com mesmo conteúdo - atualizar status se necessário
  ...
  // Atualizar duplicatas (mudança de PENDING para POSTED com ID diferente)
...
async function processItem(client, item, from, to) {
  const pluggyItem = await getPluggyItem(item.pluggy_item_id);
  const status = pluggyItem?.status ?? 'UNKNOWN';

  // NÃO aceitar UPDATING — dados ainda estão sendo coletados
  if (!['UPDATED', 'PARTIAL_SUCCESS'].includes(status)) {
  ...
  // Deleção de órfãs (opcional, controlado por env)
  if (allTx.length > 0 && !SKIP_ORPHAN_DELETE) {
    const currentIds = allTx.map(t => t.id);
    const { rowCount: deletedBank } = await pool.query(
      `DELETE FROM transactions
       WHERE pluggy_item_id = $1 AND date::date >= $2::date AND date::date <= $3::date AND id != ALL($4::text[])`,
      [item.pluggy_item_id, from, to, currentIds]
    ).catch(() => ({ rowCount: 0 }));
    const { rowCount: deletedCredit } = await pool.query(
      `DELETE FROM credit_transactions
       WHERE pluggy_item_id = $1 AND date::date >= $2::date AND date::date <= $3::date AND id != ALL($4::text[])`,
      [item.pluggy_item_id, from, to, currentIds]
    ).catch(() => ({ rowCount: 0 }));
    
    if (deletedBank > 0 || deletedCredit > 0) {
      console.log(`    🗑 ${deletedBank} transações bancárias + ${deletedCredit} crédito deletadas (órfãs)`);
    }
  } else if (SKIP_ORPHAN_DELETE) {
    console.log(`    ⏭ deleção de órfãs desabilitada (SYNC_SKIP_ORPHAN_DELETE=1)`);
  }
...
async function main() {
  ...
  for (const client of clients) {
    ...
    // ── PATCH com polling (se não estiver desabilitado) ────────────────────────
    if (!SKIP_PATCH) {
      // Usar last_sync do cliente (não do item, pois a tabela items não tem essa coluna)
      const clientLastSync = client.last_sync ? new Date(client.last_sync).getTime() : 0;
      const timeSinceLastSync = Date.now() - clientLastSync;
      
      for (const item of items) {
        // Evitar PATCH se foi syncado recentemente (menos de 5 min)
        if (timeSinceLastSync < MIN_TIME_BETWEEN_PATCHES_MS && hasTx.length > 0) {
          console.log(`  ⏭ ${client.name} / ${item.institution_name}: sync recente (${Math.round(timeSinceLastSync/1000)}s), pulando PATCH`);
          continue;
        }
        ...
        if (!patchResult) {
          console.log(`    ⚠ PATCH falhou para ${item.institution_name}`);
          continue;
        }
        
        // Se o item entrou em UPDATING, aguardar
        if (patchResult.status === 'UPDATING') {
          console.log(`    ⏳ aguardando sync completar...`);
          const readyItem = await waitForItemReady(item.pluggy_item_id);
          if (readyItem) {
            console.log(`    ✓ sync completou: ${readyItem.status}`);
          }
        }
      }
    } else {
