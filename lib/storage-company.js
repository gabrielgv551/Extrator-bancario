...
export async function addItem(pool, { id, clientId, pluggyItemId, institutionName, institutionLogo, accountNumbers }) {
  ...
  const { rows: existing } = await pool.query(
    `SELECT id FROM extrator_items WHERE client_id = $1 AND institution_name = $2 AND deleted_at IS NULL LIMIT 1`,
  ...
  if (existing.length > 0) {
    const { rows } = await pool.query(
      `UPDATE extrator_items
       SET pluggy_item_id = $1, institution_logo = $2, account_numbers = $3,
           requires_reconnect = FALSE, consecutive_errors = 0, error_code = NULL, error_message = NULL,
           status = NULL, updated_at = NOW()
       WHERE id = $4 RETURNING ${I_RETURNING}`,
...
export async function addKlaviItem(pool, { id, clientId, klaviLinkId, klaviConsentId, institutionCode, institutionName, institutionLogo, accountNumbers, businessTaxId, personalTaxId, taxType, status = 'WAITING_DATA' }) {
  ...
  if (existing.length > 0) {
    const { rows } = await pool.query(
      `UPDATE extrator_items
       SET provider = 'klavi', klavi_link_id = $1, klavi_consent_id = $2, institution_code = $3,
           institution_logo = $4, account_numbers = $5, business_tax_id = $6, personal_tax_id = $7,
           tax_type = $8, status = $9, requires_reconnect = FALSE, consecutive_errors = 0,
...
export async function getTransactionsByClientId(pool, clientId, { from, to } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT t.id, t.client_id AS "clientId", c.name AS "clientName", t.date, t.description, t.type,
            ...
            t.counterparty_document AS "counterpartyDocument", t.status, t.date_transacted AS "dateTransacted", t.synced_at AS "syncedAt", 'bank' AS source
     ...
     SELECT ct.id, ct.client_id AS "clientId", c.name AS "clientName", ct.date, ct.description, ct.type,
            ...
            ct.counterparty_document AS "counterpartyDocument", ct.status, ct.date_transacted AS "dateTransacted", ct.synced_at AS "syncedAt", 'credit' AS source
