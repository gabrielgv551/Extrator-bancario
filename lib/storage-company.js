...
import { CLASSIFICACOES } from './classification.js';
...
const C = `SELECT id, name, portal_token AS "portalToken", last_sync AS "lastSync", created_at AS "createdAt", business_tax_id AS "businessTaxId", gestor_empresa AS "gestorEmpresa", classificar_de AS "classificarDe" FROM extrator_clients`;
const C_RETURNING = `id, name, portal_token AS "portalToken", last_sync AS "lastSync", created_at AS "createdAt", business_tax_id AS "businessTaxId", gestor_empresa AS "gestorEmpresa", classificar_de AS "classificarDe"`;
...
export async function updateClient(pool, id, updates) {
  ...
  if (updates.classificarDe  !== undefined) { sets.push(`classificar_de   = $${i++}`); values.push(updates.classificarDe || null); }
...
export async function addKlaviItem(pool, { id, clientId, klaviLinkId, klaviConsentId, institutionCode, institutionName, institutionLogo, accountNumbers, businessTaxId, personalTaxId, taxType, status = 'WAITING_DATA' }) {
  ...
  if (klaviConsentId) {
    const { rows: sameConsent } = await pool.query(
      `SELECT id FROM extrator_items WHERE client_id = $1 AND LOWER(klavi_consent_id) = LOWER($2) AND deleted_at IS NULL LIMIT 1`,
  ...
  if (klaviLinkId) {
    const { rows: sameLinkRows } = await pool.query(
      `SELECT id, klavi_consent_id AS "klaviConsentId" FROM extrator_items WHERE client_id = $1 AND LOWER(klavi_link_id) = LOWER($2) AND deleted_at IS NULL ORDER BY created_at ASC LIMIT 1`,
      [clientId, klaviLinkId]
    );
    if (sameLinkRows.length > 0) {
      const sameLink = sameLinkRows[0];
      const existingConsentId = sameLink.klaviConsentId ? String(sameLink.klaviConsentId).toLowerCase() : null;
      const newConsentId = klaviConsentId ? String(klaviConsentId).toLowerCase() : null;
      // Só atualiza pelo linkId se o item ainda não tiver um consentId vinculado
      // OU se for exatamente o mesmo consentId. Isso evita que múltiplos
      // consentimentos gerados dentro de um mesmo link sobrescrevam um único item.
      if (!existingConsentId || !newConsentId || existingConsentId === newConsentId) {
        const { rows } = await pool.query(
          `UPDATE extrator_items
           SET klavi_consent_id = $1, institution_code = $2, institution_name = $3, institution_logo = $4,
               ...
               deleted_at = NULL, requires_reconnect = FALSE, consecutive_errors = 0, error_code = NULL, error_message = NULL, updated_at = NOW()
           WHERE id = $10 RETURNING ${I_RETURNING}`,
          [klaviConsentId, institutionCode, institutionName ?? null, institutionLogo ?? null, accountNumbers ?? null, businessTaxId ?? null, personalTaxId ?? null, taxType ?? null, status, sameLink.id]
        );
        return rows[0];
      }
      // Se chegou aqui, o linkId já tem um item com consentId diferente.
      // Não fazemos fallback por linkId; criamos um novo item abaixo.
    }
  }
...
export async function getItemByKlaviLinkId(pool, klaviLinkId, { includeDeleted = false } = {}) {
  const deletedFilter = includeDeleted ? '' : ' AND deleted_at IS NULL';
  const { rows } = await pool.query(`${I} WHERE LOWER(klavi_link_id) = LOWER($1)${deletedFilter}`, [klaviLinkId]);
  return rows[0] ?? null;
}

export async function getItemByKlaviConsentId(pool, klaviConsentId, { includeDeleted = false } = {}) {
  const deletedFilter = includeDeleted ? '' : ' AND deleted_at IS NULL';
  const { rows } = await pool.query(`${I} WHERE LOWER(klavi_consent_id) = LOWER($1)${deletedFilter}`, [klaviConsentId]);
  return rows[0] ?? null;
}
...
export async function upsertTransactionsBatch(pool, clientId, clientName, pluggyItemId, transactions) {
  ...
  for (let c = 0; c < transactions.length; c += CHUNK) {
    ...
    for (const tx of chunk) {
      placeholders.push(`($${p},$${p+1},$${p+2},$${p+3},$${p+4},$${p+5},$${p+6},$${p+7},$${p+8},$${p+9},$${p+10},$${p+11},$${p+12},$${p+13},$${p+14},$${p+15},$${p+16},$${p+17},$${p+18},$${p+19},$${p+20},$${p+21},$${p+22},$${p+23},NOW())`);
      params.push(tx.id, clientId, clientName ?? null, pluggyItemId, tx.date, tx.description ?? '', tx.type,
        ...
        tx.companyName ?? null, tx.companyCnpj ?? null,
    ...
    await pool.query(
      `INSERT INTO extrator_transactions
         (id, client_id, client_name, pluggy_item_id, date, description, type, amount, balance,
          category, category_l1, category_l2, category_l3, account_name, account_number, account_type, institution_name, company_name, company_cnpj, counterparty_name, counterparty_document, status, date_transacted, api_order, synced_at)
...
export async function getTransactionsByClientId(pool, clientId, { from, to } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT t.id, t.client_id AS "clientId", c.name AS "clientName", t.pluggy_item_id AS "pluggyItemId", t.date, t.description, t.type,
            ...
            t.classificacao_l1 AS "classificacaoL1", t.classificacao_l2 AS "classificacaoL2",
            t.account_name AS "accountName", t.account_number AS "accountNumber", t.account_type AS "accountType",
            t.institution_name AS "institutionName", t.company_name AS "companyName", t.company_cnpj AS "companyCnpj",
     ...
     SELECT ct.id, ct.client_id AS "clientId", c.name AS "clientName", ct.pluggy_item_id AS "pluggyItemId", ct.date, ct.description, ct.type,
            ...
            ct.classificacao_l1 AS "classificacaoL1", ct.classificacao_l2 AS "classificacaoL2",
            ct.account_name AS "accountName", ct.account_number AS "accountNumber", ct.account_type AS "accountType",
            ct.institution_name AS "institutionName", ct.company_name AS "companyName", ct.company_cnpj AS "companyCnpj",
            ct.counterparty_name AS "counterpartyName",
...
// Atualiza a classificação manual (Receita/Despesa) de uma transação.
// Retorna true se a transação pertence ao cliente e foi atualizada.
export async function updateTransactionClassification(pool, clientId, txId, classificacaoL1, classificacaoL2) {
  const { rowCount: bank } = await pool.query(
    `UPDATE extrator_transactions
     SET classificacao_l1 = $3, classificacao_l2 = $4
     WHERE id = $1 AND client_id = $2`,
    [txId, clientId, classificacaoL1, classificacaoL2]
  );
  if (bank > 0) return true;
  const { rowCount: credit } = await pool.query(
    `UPDATE extrator_credit_transactions
     SET classificacao_l1 = $3, classificacao_l2 = $4
     WHERE id = $1 AND client_id = $2`,
    [txId, clientId, classificacaoL1, classificacaoL2]
  );
...
export async function upsertTransactions(pool, clientId, clientName, pluggyItemId, transactions) {
  ...
  for (const tx of transactions) {
    await pool.query(
      `INSERT INTO extrator_transactions
         (id, client_id, client_name, pluggy_item_id, date, description, type, amount, balance, category, category_l1, category_l2, category_l3, account_name, account_number, account_type, institution_name, company_name, company_cnpj, counterparty_name, counterparty_document, status, date_transacted, api_order, synced_at)
...
export async function getTransactionsByIds(pool, ids) {
  ...
  const { rows } = await pool.query(
    `SELECT id, client_id AS "clientId", pluggy_item_id AS "pluggyItemId", date, description, type,
            ...
            company_name AS "companyName", company_cnpj AS "companyCnpj",
...
// ── Plano de contas (2 níveis: grupo -> conta) ────────────────────────────────

export async function listPlanoContas(pool, clientId) {
  const { rows } = await pool.query(
    `SELECT id, client_id AS "clientId", nome, parent_id AS "parentId", ativo,
            created_at AS "createdAt", updated_at AS "updatedAt"
     FROM extrator_plano_contas
     WHERE client_id = $1
...
// Se o cliente ainda não tem plano de contas, semeia com a lista estática
// de CLASSIFICACOES (mesmos nomes das classificações já existentes).
export async function seedPlanoContasIfEmpty(pool, clientId) {
  const { rows } = await pool.query(
    `SELECT COUNT(*)::int AS total FROM extrator_plano_contas WHERE client_id = $1`,
    [clientId]
  );
  if (rows[0].total > 0) return false;
  for (const [grupo, contas] of Object.entries(CLASSIFICACOES)) {
    const g = await pool.query(
      `INSERT INTO extrator_plano_contas (client_id, nome) VALUES ($1, $2) RETURNING id`,
      [clientId, grupo]
    );
    for (const conta of contas) {
      await pool.query(
        `INSERT INTO extrator_plano_contas (client_id, nome, parent_id) VALUES ($1, $2, $3)`,
        [clientId, conta, g.rows[0].id]
      );
    }
  }
  return true;
...
export async function createPlanoConta(pool, clientId, { nome, parentId }) {
  const nomeTrim = (nome || '').trim();
  if (!nomeTrim) throw new Error('Nome é obrigatório');
  if (parentId) {
    const { rows } = await pool.query(
      `SELECT id FROM extrator_plano_contas WHERE id = $1 AND client_id = $2 AND parent_id IS NULL`,
      [parentId, clientId]
    );
    if (rows.length === 0) throw new Error('Grupo não encontrado para este cliente');
  }
  const { rows } = await pool.query(
    `INSERT INTO extrator_plano_contas (client_id, nome, parent_id)
     VALUES ($1, $2, $3)
     RETURNING id, client_id AS "clientId", nome, parent_id AS "parentId", ativo`,
    [clientId, nomeTrim, parentId || null]
  );
  return rows[0];
}

export async function updatePlanoConta(pool, clientId, contaId, { nome, ativo }) {
  const sets = ['updated_at = NOW()'];
  const values = [contaId, clientId];
  let i = 3;
  if (nome !== undefined) {
    const nomeTrim = (nome || '').trim();
    if (!nomeTrim) throw new Error('Nome é obrigatório');
  ...
  const { rows } = await pool.query(
    `UPDATE extrator_plano_contas SET ${sets.join(', ')}
     WHERE id = $1 AND client_id = $2
     RETURNING id, client_id AS "clientId", nome, parent_id AS "parentId", ativo`,
    values
  );
...
export async function deletePlanoConta(pool, clientId, contaId) {
  const { rowCount } = await pool.query(
    `DELETE FROM extrator_plano_contas WHERE id = $1 AND client_id = $2`,
    [contaId, clientId]
  );
  return rowCount > 0;
}
