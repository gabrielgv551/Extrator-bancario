...
export async function getAllTransactions(itemId, { from, to, createdAtFrom } = {}) {
  ...
  // apiOrder preserva a ordem de retorno da API para desempatar transações
  // do mesmo dia/horário e manter a sequência do extrato / saldo.
  let apiOrder = 0;
  const perAccount = [];
  ...
  allTx.sort((a, b) => {
    ...
    return (a.apiOrder || 0) - (b.apiOrder || 0);
  });
  return allTx;
}
