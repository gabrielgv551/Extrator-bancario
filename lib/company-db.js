...
export async function listActiveCompanies() {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT slug, nome FROM empresas WHERE status = 'ativo' ORDER BY slug`
    );
    return rows.map((r) => ({ slug: r.slug, name: r.nome || r.slug }));
  } finally {
    await pool.end();
  }
}
...
export async function forEachCompany(callback) {
  ...
  for (const company of companies) {
    try {
      ...
      results.push(await callback({ empresa: company.slug, pool }));
    } catch (err) {
      console.error(`[company-db] erro processando empresa ${company.slug}:`, err.message);
      results.push({ empresa: company.slug, error: err.message });
    }
  }
  return results;
}
