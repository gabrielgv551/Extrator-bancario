...
async function syncKlaviItems({ empresa, pool, client, items, results, lockId }) {
  ...
  for (const item of klaviItems) {
    ...
    // Conexão ainda não concluída no portal (sem institutionCode e sem consentimento):
    // não há o que solicitar à Klavi — é skip, não falha. Ex.: placeholder "Banco em
    // conexão" ou link criado mas fluxo de consentimento não finalizado.
    if (!item.institutionCode && !item.klaviConsentId) {
      results.push({
        empresa,
        client: client.name,
        bank: item.institutionName,
        success: true,
        status: 'SKIPPED',
        note: 'Conexão ainda não concluída',
      });
      continue;
    }
