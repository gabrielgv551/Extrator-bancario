...
export async function runCompanySetupQueries(db) {
  ...
  // Garante colunas do schema atual em bancos legados multi-tenant.
  // Todos os ALTER TABLEs abaixo sao idempotentes e servem como seguranca
  // para empresas criadas antes da adicao de novas colunas.
  await db.query(`ALTER TABLE extrator_clients ADD COLUMN IF NOT EXISTS business_tax_id VARCHAR(14)`);
  await db.query(`ALTER TABLE extrator_clients ADD COLUMN IF NOT EXISTS gestor_empresa VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_clients ADD COLUMN IF NOT EXISTS portal_token VARCHAR(64)`);
  ...
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS account_numbers TEXT`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS status VARCHAR(50)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS execution_status VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS error_code VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS error_message TEXT`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS last_updated_at TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS last_error_at TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS sync_count INTEGER DEFAULT 0`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS consecutive_errors INTEGER DEFAULT 0`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS requires_reconnect BOOLEAN DEFAULT FALSE`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS consent_expires_at TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS notification_sent_at TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS updated_at TIMESTAMPTZ DEFAULT NOW()`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS provider VARCHAR(20) DEFAULT 'pluggy'`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS klavi_link_id VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS klavi_consent_id VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS business_tax_id VARCHAR(14)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS personal_tax_id VARCHAR(11)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS tax_type VARCHAR(10)`);
  await db.query(`ALTER TABLE extrator_items ADD COLUMN IF NOT EXISTS institution_code VARCHAR(10)`);

  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS pluggy_item_id VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS institution_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS counterparty_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS counterparty_document VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS company_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS company_cnpj VARCHAR(14)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS date_transacted TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS account_number VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS client_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS category_l1 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS category_l2 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS category_l3 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS account_type VARCHAR(50)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS api_order INTEGER`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS classificacao_l1 VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_transactions ADD COLUMN IF NOT EXISTS classificacao_l2 VARCHAR(100)`);

  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS institution_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS account_type VARCHAR(50)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS counterparty_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS counterparty_document VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS company_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS company_cnpj VARCHAR(14)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS date_transacted TIMESTAMPTZ`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS account_number VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS client_name VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS category_l1 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS category_l2 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS category_l3 VARCHAR(255)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS api_order INTEGER`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS classificacao_l1 VARCHAR(100)`);
  await db.query(`ALTER TABLE extrator_credit_transactions ADD COLUMN IF NOT EXISTS classificacao_l2 VARCHAR(100)`);

  // Plano de contas do cliente (2 níveis: grupo -> conta).
  await db.query(`
    CREATE TABLE IF NOT EXISTS extrator_plano_contas (
      id         UUID PRIMARY KEY DEFAULT gen_random_uuid(),
      client_id  UUID NOT NULL REFERENCES extrator_clients(id) ON DELETE CASCADE,
      nome       VARCHAR(255) NOT NULL,
      parent_id  UUID REFERENCES extrator_plano_contas(id) ON DELETE CASCADE,
      ativo      BOOLEAN DEFAULT TRUE,
      created_at TIMESTAMPTZ DEFAULT NOW(),
      updated_at TIMESTAMPTZ DEFAULT NOW()
    )
  `);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_plano_contas_client_parent ON extrator_plano_contas(client_id, parent_id)`);

  // Índices (após ALTERs para que colunas legadas já existam).
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_transactions_client_date ON extrator_transactions(client_id, date DESC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_transactions_client_account_date_order ON extrator_transactions(client_id, account_number, date DESC, api_order ASC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_transactions_item ON extrator_transactions(pluggy_item_id)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_credit_transactions_client_date ON extrator_credit_transactions(client_id, date DESC)`);
  await db.query(`CREATE INDEX IF NOT EXISTS idx_extrator_credit_transactions_client_account_date_order ON extrator_credit_transactions(client_id, account_number, date DESC, api_order ASC)`);

  await db.query(`ALTER TABLE extrator_debts ADD COLUMN IF NOT EXISTS institution_name VARCHAR(255)`);
