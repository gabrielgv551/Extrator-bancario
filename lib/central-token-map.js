...
import pg from 'pg';
import { listActiveCompanies, getCompanyPool } from './company-db';
...
/**
 * Resolve empresa pelo item, primeiro no mapa central e, se falhar,
 * fazendo uma busca nos bancos de cada empresa ativa. Isso evita que
 * webhooks sejam perdidos quando itens são criados fora do fluxo do portal.
 */
export async function getEmpresaByItemWithFallback({ pluggyItemId, klaviLinkId, klaviConsentId }) {
  const cached = await getEmpresaByItem({ pluggyItemId, klaviLinkId, klaviConsentId });
  if (cached) return cached;

  const identifiers = [];
  if (klaviLinkId) identifiers.push({ key: 'klavi_link_id', value: klaviLinkId });
  if (klaviConsentId) identifiers.push({ key: 'klavi_consent_id', value: klaviConsentId });
  if (pluggyItemId) identifiers.push({ key: 'pluggy_item_id', value: pluggyItemId });
  if (identifiers.length === 0) return null;

  const companies = await listActiveCompanies();
  for (const company of companies) {
    try {
      const pool = await getCompanyPool(company.slug);
      for (const { key, value } of identifiers) {
        const { rows } = await pool.query(
          `SELECT id FROM extrator_items WHERE ${key} = $1 AND deleted_at IS NULL LIMIT 1`,
          [value]
        );
        if (rows.length > 0) {
          console.log('[central-token-map] empresa resolvida por fallback para item %s (%s): %s', rows[0].id, key, company.slug);
          return company.slug;
        }
      }
    } catch (err) {
      console.warn('[central-token-map] falha ao buscar item na empresa %s:', company.slug, err.message);
    }
  }
