...
export async function getEmpresaByItem({ pluggyItemId, klaviLinkId, klaviConsentId }) {
  ...
  const values = [];
  let i = 1;
  if (pluggyItemId) { conditions.push(`LOWER(pluggy_item_id) = LOWER($${i++})`); values.push(pluggyItemId); }
  if (klaviLinkId) { conditions.push(`LOWER(klavi_link_id) = LOWER($${i++})`); values.push(klaviLinkId); }
  if (klaviConsentId) { conditions.push(`LOWER(klavi_consent_id) = LOWER($${i++})`); values.push(klaviConsentId); }
