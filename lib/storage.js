  ...
  for (const tx of transactions) {
    await pool.query(
      `INSERT INTO transactions
         (id, client_id, pluggy_item_id, date, description, type, amount, balance, category, account_name, account_type, status, synced_at)
  ...
  for (const tx of transactions) {
    await pool.query(
      `INSERT INTO credit_transactions
         (id, client_id, pluggy_item_id, date, description, type, amount, balance, category, account_name, status, synced_at)
      ...
      [tx.id, clientId, pluggyItemId, tx.date, tx.description ?? '', tx.type,
       tx.amount, tx.balance ?? null, tx.category ?? null, tx.accountName ?? null, tx.status ?? null]
    );
    count++;
  }
  return count;
...
export async function getTransactionsByClientId(clientId, { from, to } = {}) {
  ...
  if (from) { where += ` AND date::date >= $${i++}`; values.push(from); }
  if (to)   { where += ` AND date::date <= $${i++}`; values.push(to); }
