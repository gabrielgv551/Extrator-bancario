...
export async function updateItemStatus(id, {
  ...
  if (institutionName !== undefined) { sets.push(`institution_name = $${i++}`); values.push(institutionName); }
  if (institutionLogo !== undefined) { sets.push(`institution_logo = $${i++}`); values.push(institutionLogo); }
...
export async function getTransactionsByIds(ids) {
  ...
  const { rows } = await getPool().query(
    `SELECT id, client_id AS "clientId", pluggy_item_id AS "pluggyItemId", date, description, type,
     ...
     ORDER BY date DESC, api_order ASC NULLS LAST`,
    [ids]
  );
  return rows;
}
...
export async function getTransactionsByClientId(clientId, { from, to } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT t.id, t.client_id AS "clientId", c.name AS "clientName", t.date, t.description, t.type,
            ...
            t.status, t.date_transacted AS "dateTransacted", t.api_order AS "apiOrder", t.synced_at AS "syncedAt", 'bank' AS source
     FROM transactions t
     ...
     SELECT ct.id, ct.client_id AS "clientId", c.name AS "clientName", ct.date, ct.description, ct.type,
            ...
            ct.status, ct.date_transacted AS "dateTransacted", ct.api_order AS "apiOrder", ct.synced_at AS "syncedAt", 'credit' AS source
