...
const C = `SELECT id, name, portal_token AS "portalToken", last_sync AS "lastSync", created_at AS "createdAt", gestor_empresa AS "gestorEmpresa" FROM clients`;
...
export async function createClient({ id, name, portalToken }) {
  const { rows } = await getPool().query(
    `INSERT INTO clients (id, name, portal_token, last_sync, created_at)
     ...
     RETURNING id, name, portal_token AS "portalToken", last_sync AS "lastSync", created_at AS "createdAt", gestor_empresa AS "gestorEmpresa"`,
    [id, name, portalToken]
  );
  return rows[0];
}
...
export async function addItem({ id, clientId, pluggyItemId, institutionName, institutionLogo, accountNumbers }) {
    ...
    const { rows } = await pool.query(
      `UPDATE items
       ...
       RETURNING id, client_id AS "clientId", pluggy_item_id AS "pluggyItemId",
                 institution_name AS "institutionName", institution_logo AS "institutionLogo",
                 account_numbers AS "accountNumbers", created_at AS "createdAt"`,
  ...
  const { rows } = await pool.query(
    `INSERT INTO items (id, client_id, pluggy_item_id, institution_name, institution_logo, account_numbers, created_at)
