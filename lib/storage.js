  ...
  for (const tx of transactions) {
    await pool.query(
      `INSERT INTO transactions
         (id, client_id, pluggy_item_id, date, description, type, amount, balance, category, category_l1, category_l2, category_l3, account_name, account_number, account_type, institution_name, counterparty_name, counterparty_document, status, date_transacted, synced_at)
...
export async function getTransactionsByClientId(clientId, { from, to } = {}) {
  ...
  const { rows } = await pool.query(
    `SELECT id, client_id AS "clientId", date, description, type,
            ...
            category_l1 AS "categoryL1", category_l2 AS "categoryL2", category_l3 AS "categoryL3",
