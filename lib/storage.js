...
export async function getItemById(id, { includeDeleted = false } = {}) {
  const deletedFilter = includeDeleted ? '' : ' AND deleted_at IS NULL';
  const { rows } = await getPool().query(`${I} WHERE id::text = $1${deletedFilter}`, [String(id)]);
  return rows[0] ?? null;
}
...
export async function addKlaviItem({ id, clientId, klaviLinkId, klaviConsentId, institutionCode, institutionName, institutionLogo, accountNumbers, businessTaxId, status = 'WAITING_DATA' }) {
  const pool = getPool();
  ...
  if (sameConsent.length > 0) {
    const { rows } = await pool.query(
      `UPDATE items
       ...
       WHERE id = $8
       RETURNING ${I_RETURNING}`,
      [klaviLinkId, institutionCode, institutionName ?? null, institutionLogo ?? null, accountNumbers ?? null, businessTaxId ?? null, status, sameConsent[0].id]
    );
    return rows[0];
  }
  ...
  if (existing.length > 0) {
    const { rows } = await pool.query(
      `UPDATE items
       ...
       WHERE id = $8
       RETURNING ${I_RETURNING}`,
      [klaviLinkId, klaviConsentId, institutionCode, institutionLogo ?? null, accountNumbers ?? null, businessTaxId ?? null, status, existing[0].id]
    );
    return rows[0];
  }

  const { rows } = await pool.query(
    `INSERT INTO items (id, client_id, provider, klavi_link_id, klavi_consent_id, institution_code, institution_name, institution_logo, account_numbers, business_tax_id, status, created_at)
     VALUES ($1, $2, 'klavi', $3, $4, $5, $6, $7, $8, $9, $10, NOW())
     RETURNING ${I_RETURNING}`,
    [id, clientId, klaviLinkId, klaviConsentId, institutionCode, institutionName ?? null, institutionLogo ?? null, accountNumbers ?? null, businessTaxId ?? null, status]
  );
  return rows[0];
}
