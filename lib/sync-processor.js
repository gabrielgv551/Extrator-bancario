...
export async function fetchAndPersistTransactions(clientId, pluggyItemId, { from, to, createdAtFrom } = {}) {
  ...
  const savedBank = await upsertTransactionsBatch(clientId, clientName, pluggyItemId, bankTx);
  const savedCredit = await upsertCreditTransactionsBatch(clientId, clientName, pluggyItemId, creditTx);
