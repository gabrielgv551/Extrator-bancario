...
export const DEFAULT_KLAVI_PRODUCTS = ['pj_categorized_checking_l3'];
...
export function mapKlaviReportToLocal({ productName, report, institutionCode, institutionName }) {
  ...
  console.log('[mapKlaviReportToLocal] bankTransactions sample:', JSON.stringify(bankTransactions[0], null, 2).slice(0, 600));
...
export function normalizeKlaviStatus(reportPayload, consentStatus) {
  const hasData = !!(reportPayload?.checkingAccounts?.length || reportPayload?.creditCardAccounts?.length || reportPayload?.savingsAccounts?.length);
