...
import { recordKlaviRequestLog } from './storage-company.js';
...
function extractQueryFromPath(path) {
...
async function fetchKlaviRaw(path, options = {}, { timeoutMs = DEFAULT_TIMEOUT_MS, retries = MAX_RETRIES, accessToken = null, logMeta = null } = {}) {
  ...
  async function writeLog({ responseStatus, responseBody, errorMessage }) {
    if (!logMeta || !logMeta.pool) return;
    try {
      const durationMs = Date.now() - startedAt;
      const method = options.method || 'GET';
      const query = extractQueryFromPath(path);
      ...
      await recordKlaviRequestLog(logMeta.pool, {
        ...
        responseBody,
        durationMs,
        linkId: logMeta.linkId ?? null,
        consentId: logMeta.consentId ?? null,
        personalTaxId: logMeta.personalTaxId ?? null,
        businessTaxId: logMeta.businessTaxId ?? null,
        institutionCode: logMeta.institutionCode ?? null,
        errorMessage: errorMessage ? String(errorMessage).slice(0, 2000) : null,
        clientId: logMeta.clientId ?? null,
        itemId: logMeta.itemId ?? null,
      });
    } catch (err) {
      console.error('[klavi] falha ao gravar log de requisição:', err.message);
    }
  }
    ...
    try {
      ...
      if (res.ok) {
        if (res.status === 204 || res.headers.get('content-length') === '0') {
          await writeLog({ responseStatus: res.status, responseBody: null });
          return { ok: true, data: null };
        }
        const data = await res.json();
        await writeLog({ responseStatus: res.status, responseBody: data });
        return { ok: true, data };
      }
      ...
      if (!isRetryable(res.status)) {
        ...
        await writeLog({ responseStatus: res.status, responseBody: errBody, errorMessage: lastError.message });
        throw lastError;
      }
      ...
      if (!isRetryable(lastError.status)) {
        await writeLog({ responseStatus: lastError.status, responseBody: lastError.body ?? null, errorMessage: lastError.message });
        throw lastError;
      }
      if (attempt < retries) {
...
import { resolveInstitutionNameByCode as resolveInstitutionNameByCodeBase } from './institution-names.js';

export function resolveInstitutionNameByCode(institutionCode) {
  return resolveInstitutionNameByCodeBase(institutionCode);
}

// ── Links ────────────────────────────────────────────────────────────────────

function buildLogMeta({ pool, source, clientId, itemId, linkId, consentId, personalTaxId, businessTaxId, institutionCode }) {
  if (!pool) return null;
  return { pool, source: source ?? null, clientId, itemId, linkId, consentId, personalTaxId, businessTaxId, institutionCode };
}

export async function createLink({ personalTaxId, businessTaxId, redirectUrl, productsCallbackUrl, externalInfo } = {}, { pool, source, clientId, itemId } = {}) {
  ...
  const logMeta = buildLogMeta({ pool, source: source || 'portal', clientId, itemId, personalTaxId, businessTaxId });
...
export async function getInstitutions(linkToken, { pool, source, clientId, itemId, linkId } = {}) {
  const logMeta = buildLogMeta({ pool, source: source || 'portal', clientId, itemId, linkId });
...
export async function createConsent({ linkToken, personalTaxId, businessTaxId, institutionCode, redirectUrl, externalTrackId, email, phone } = {}, { pool, source, clientId, itemId, linkId } = {}) {
  ...
  const logMeta = buildLogMeta({ pool, source: source || 'portal', clientId, itemId, linkId, personalTaxId, businessTaxId, institutionCode });
...
export async function getConsentList({ personalTaxId, businessTaxId, linkId } = {}, { pool, source, clientId, itemId, consentId } = {}) {
  const accessToken = await getAccessToken();
  ...
  const logMeta = buildLogMeta({ pool, source: source || 'debug', clientId, itemId, linkId, consentId, personalTaxId, businessTaxId });
  return fetchKlavi(`/consents${query}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  }, { logMeta });
}

export async function deleteConsent(consentId, { pool, source, clientId, itemId, linkId } = {}) {
  const accessToken = await getAccessToken();
  const logMeta = buildLogMeta({ pool, source: source || 'portal', clientId, itemId, linkId, consentId });
...
export async function requestBusinessInstitutionData({
  ...
  const logMeta = buildLogMeta({ pool, source: source || 'webhook', clientId, itemId, linkId, consentId: consentIds?.[0], businessTaxId, institutionCode });
...
export async function requestPersonalInstitutionData({
  ...
  const logMeta = buildLogMeta({ pool, source: source || 'webhook', clientId, itemId, linkId, consentId: consentIds?.[0], personalTaxId, institutionCode });
...
export async function getActiveKlaviConsent({ item, businessTaxId, personalTaxId, pool, source, clientId, itemId: explicitItemId }) {
  const filters = [];
  ...
  for (const params of filters) {
    const list = await getConsentList(params, logMeta);
...
export function mapKlaviReportToLocal({ productName, report, institutionCode, institutionName }) {
  ...
  const name = institutionName || resolveInstitutionNameByCode(institutionCode) || report?.checkingAccounts?.[0]?.brandName || 'Banco';

  // Conta corrente / poupança
  const checkingAccounts = report?.checkingAccounts || [];
  const savingsAccounts = report?.savingsAccounts || [];
  // Algumas instituições (ex: C6 Bank) podem entregar conta corrente em arrays genéricos.
  const genericAccounts = (report?.accounts || []).filter(a =>
    !['CREDIT_CARD', 'CREDIT'].includes(String(a.subtype || a.type).toUpperCase())
  );
  const paymentAccounts = report?.paymentAccounts || [];
  const bankSources = [
    ...checkingAccounts.map(a => ({ ...a, _source: 'checkingAccounts' })),
    ...savingsAccounts.map(a => ({ ...a, _source: 'savingsAccounts' })),
    ...genericAccounts.map(a => ({ ...a, _source: 'accounts' })),
    ...paymentAccounts.map(a => ({ ...a, _source: 'paymentAccounts' })),
  ];
  // Preserva a ordem de retorno da API Klavi para desempatar transações do mesmo dia.
  let apiOrder = 0;
  for (const acc of bankSources) {
    ...
    for (const tx of acc.transactionDetails || []) {
      ...
      bankTransactions.push({
        ...
        companyCnpj: acc.companyCnpj || null,
    ...
    for (const stmt of statements) {
      for (const tx of stmt.transactionDetails || []) {
        ...
        creditTransactions.push({
          ...
          companyCnpj: card.companyCnpj || null,
          counterpartyName: null,
          counterpartyDocument: tx.partieCnpjCpf || null,
...
export function normalizeKlaviStatus(reportPayload, consentStatus) {
  const hasData = !!(
    ...
    reportPayload?.accounts?.length ||
    reportPayload?.paymentAccounts?.length ||
...
// ── Institution name helpers ─────────────────────────────────────────────────

const PLACEHOLDER_INSTITUTION_NAMES = [
...
export function isPlaceholderInstitutionName(name) {
  ...
  return PLACEHOLDER_INSTITUTION_NAMES.some(p => normalized.localeCompare(p, 'pt-BR', { sensitivity: 'base' }) === 0);
}
